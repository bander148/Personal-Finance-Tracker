...
class TransactionRepository:
    ...
    def get_by_date(self, tr_date: date,user_id : int) -> List[Transaction]:
        return self.db.query(Transaction).filter(Transaction.date == tr_date,Transaction.user_id == user_id).all()

    def get_by_date_range(self, start_date: date, end_date: date,user_id : int) -> List[Transaction]:
    ...
    def get_by_category_id(self, category_id: int,user_id : int) -> List[Transaction]:
        return self.db.query(Transaction).filter(Transaction.category_id == category_id,Transaction.user_id==user_id).all()
    ...
    def get_by_type(self, transaction_type: str,user_id : int) -> List[Transaction]:
        return self.db.query(Transaction).filter(Transaction.type == transaction_type,Transaction.user_id == user_id).all()
